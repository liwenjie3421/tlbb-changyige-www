<template>
  <div id="serach">
    <hea-title title="自定义搜索条件">
      <router-link to="/searchConfig" slot="left">
        <m-button icon="back">返回</m-button>
      </router-link>
    </hea-title>
    <checklist id="checklist" title="选中的条件将会用于搜索，请点击配置" v-model="searchItems" :options="options">
    </checklist>
    <div style="text-align: center; padding-top: 10px;">
      <m-button size="small" type="primary" @click="toSearchConfig">
        去配置
      </m-button>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      toSearchConfig() {
        this.$router.push('searchConditions');
